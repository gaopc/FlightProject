...
-(void)selectDate:(int)date {
    if (date <= 0 || date > [currentMonth numDaysInMonth]) {
        return;
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    int haveDay = [self.currentMonth numDaysInMonth];
	for (int i=firstWeekDay; i<haveDay + firstWeekDay; i++) {
...
//			
		...
		if (selectedDate && i==selectedDateBlock && [currentMonth month] == [self.selectMonth month] && [currentMonth year] == [self.selectMonth year]) {
...
#pragma mark - Init
-(id)initWithNSDate:(NSDate *)adate allowShowMonths:(NSInteger)monthCount withDayCount:(NSInteger)dayCount withPushBackDay:(NSInteger)pushBackDay withSelectDate:(NSDate *)sdate{
...
//		[recognizer release];
		...
		if (([currentMonth month:adate] == month) && ([currentMonth day:adate]<day)){
            [self selectDate:[currentMonth day:sdate]];
		}else{
			[self selectDate:[currentMonth day:sdate]];
		}
		isAnimating= NO;
...
//		[self addSubview:rightArrowsImageView];
		...
		weekImageView=[UIImageView ImageViewWithFrame:CGRectMake(0.0f, 40.0f, ViewWidth, 21.0f)];
