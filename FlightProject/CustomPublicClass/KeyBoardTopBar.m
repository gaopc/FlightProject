...
//                                                                        action:nil];
        ...
        UIToolbar * _toolBarview = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 600, ViewWidth, 44)];
...
-(void)HiddenKeyBoard{
   ...
   [self.currentTextField resignFirstResponder];
    ...
    if (frame.origin.y != 0) 
    {
	frame.origin.y = IOS7_OR_LATER?64:0;
...
-(void)moveInputBarWithKeyboardHeight:(CGFloat)keybardheight withDuration:(NSTimeInterval)duretion
{
    ...
    CGFloat textField_opposite_Y = self.currentTextField.frame.origin.y;
	...
	if(IOS7_OR_LATER)
	{
		if (self.currentView.frame.size.height > 480) {
			iso7_y = 64;
		}else{
			iso7_y = 44;
		}
	}
     if (textField_opposite_Y <iso7_y) {
        CGRect frame = self.currentView.frame;
        frame.origin.y =  frame.origin.y - textField_opposite_Y +10 + (IOS7_OR_LATER?64:0);
        [UIView beginAnimations:@"ResizeForKeyboard" context:nil];
